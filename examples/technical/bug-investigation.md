# Example: Bug Investigation — Quick → Standard Upgrade
...
**Outcome:** Quick fix failed -- memory limit increase didn't solve upload crashes. Upgrading to Standard found the real cause: a refactor replaced streaming with buffering. Fixed in 1 day.
...
**Upgrade trigger:** Fix didn't work. Root cause unclear. Unknown territory.

→ **Level up to Standard mode.**

---

## Level Up: Standard Mode
...
This example shows natural upgrade path:

1. **Quick mode** — tried the obvious fix (increase memory limit)
2. **Upgrade** — fix didn't work, unknowns emerged
3. **Standard mode** — research found the root cause (buffering regression), systematic fix

Upgrade cost: ~30 minutes extra. Value: found and fixed the actual problem instead of applying a band-aid.

---

*Start Quick. Level up when you need to. The framework adapts to the problem.*
